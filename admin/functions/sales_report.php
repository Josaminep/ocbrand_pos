...
$loggedUser = (int)($_SESSION['user_id'] ?? 0); // ✅ use user_id like your other pages
...
$q = $conn->query("
    SELECT 
        ...
        si.vat,
        si.profit,
        s.payment_method AS payment_method   -- ✅ FROM SALES TABLE
    FROM sales_items si
...
.totals { margin-top:20px; background:#fff8dc; padding:16px 20px; border-radius:12px;
          box-shadow:0 4px 15px rgba(0,0,0,0.05); max-width:400px; margin-left:auto; }
.totals-row { display:flex; justify-content:space-between; padding:6px 0; font-size:16px; }

@media print {
    .filter-bar, .actions { display:none; }
    body { padding:20px; }
}
</style>
</head>
...
<div class="actions">
    ...
    <a href="sales_report_pdf.php?from=<?= urlencode($from) ?>&to=<?= urlencode($to) ?>&cashier=<?= urlencode($cashier) ?>"
       target="_blank" class="btn-print">
        <i class="fas fa-print"></i> Print PDF
    </a>

<a href="sales_report_excel.php?from=<?= urlencode($from) ?>&to=<?= urlencode($to) ?>&cashier=<?= urlencode($cashier) ?>"
   class="btn-print" style="background:#198754;">
...
<form class="filter-bar" method="GET">
    ...
    <?php if (!$isCashier): ?>
        <select name="cashier">
            ...
            <?php
            $cq = $conn->query("
                SELECT DISTINCT a.id, CONCAT(a.fname,' ',a.lname) AS fullname
                FROM sales s
            ...
            ?>
                <option value="<?= $c['id'] ?>" <?= ((string)$cashier === (string)$c['id']) ? 'selected' : '' ?>>
...
<div class="card">
    <table>
        ...
        <?php else: foreach($items as $i): ?>
            <tr>
                ...
                <td class="text-right" style="color:<?= ((float)$i['profit'] < 0) ? '#dc3545' : '#28a745' ?>">
                    ₱<?= number_format((float)$i['profit'],2) ?>
                </td>
                <td class="text-right"><?= htmlspecialchars($i['payment_method'] ?? '—') ?></td>
            </tr>
    ...
    <div class="totals">
        ...
        <div class="totals-row">
            <span>Total Vatable:</span>
            <span>₱<?= number_format($totalVatable,2) ?></span>
        </div>
        <div class="totals-row">
            <span>Total 12% VAT:</span>
