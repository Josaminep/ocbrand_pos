...
$q = $conn->query("
    SELECT
        ...
        si.vat,
        si.profit,
        s.payment_method,            -- ✅ GET FROM SALES TABLE
        s.created_at
    FROM sales_items si
        ...
        <table>
            ...
            <?php foreach ($data['items'] as $i): ?>
                <tr>
                    ...
                    <td class="amount">₱<?= number_format((float)$i['price'],2) ?></td>
                    <td class="amount">₱<?= number_format((float)$i['srp'],2) ?></td>
                    <td class="center"><?= (int)$i['quantity'] ?></td>
                    <td class="amount">₱<?= number_format((float)$i['subtotal'],2) ?></td>
                    <td class="amount">₱<?= number_format((float)$i['vatable'],2) ?></td>
                    <td class="amount">₱<?= number_format((float)$i['vat'],2) ?></td>
                    <td class="amount" style="color:<?= ((float)$i['profit'] < 0) ? '#dc3545' : '#28a745' ?>">
                        ₱<?= number_format((float)$i['profit'],2) ?>
                    </td>
                    <td><?= htmlspecialchars($i['payment_method'] ?? '—') ?></td> <!-- ✅ PRINT IT -->
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
